# `mpc00` install and remote LUKS unlock

This machine uses:
- `disko` GPT layout from `disk-config.nix`
- LUKS root device named `cryptroot`
- initrd SSH unlock on port `2222` (DHCP)
- Static address `192.168.74.11` via UDM DHCP reservation

## Manage with Colmena

`mpc00` is defined in `nix/colmena/managed.nix`.
...
nix run github:zhaofengli/colmena -- --config ./nix/colmena/managed.nix eval -E '{ nodes, ... }: nodes.mpc00.config.deployment.targetHost'
```

Deploy to `mpc00`:

```bash
nix run github:zhaofengli/colmena -- --config ./nix/colmena/managed.nix apply --on mpc00
```

Notes:
- Node deploy metadata uses `targetHost = "mpc00"` and `targetUser = "nixos"`.
- If your network does not resolve `mpc00`, use `192.168.74.11` directly
...
nix run github:nix-community/nixos-anywhere -- \
  --extra-files "$temp" \
  --flake /home/framework/code/home#mpc00 \
  --target-host root@<MPC00_IP> \
